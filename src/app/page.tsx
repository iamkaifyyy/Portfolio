...
import { OnewaveFooter } from "@/components/ui/onewave-footer";
import { VisitorCounter } from "@/components/ui/visitor-counter";
import { Sparkles } from "lucide-react";

function AvatarCard({ isInitialFlipped }: { isInitialFlipped: boolean }) {
  return (
...
export default function Home() {
  ...
  const [stage, setStage] = useState<"loading" | "centered" | "settled">("loading");
  ...
  useEffect(() => {
    ...
    // Simple "Come & Go": Centered splash appears for 1.2s then smoothly exits to main page
  ...
  return (
    <>
      {/* Solid Black Curtain to Prevent Pre-Render Flash */}
      {stage === "loading" && (
        <div className="fixed inset-0 z-50 bg-black pointer-events-none" />
      )}

      {/* Simple "Come and Go" Centered Intro Splash Overlay */}
      <AnimatePresence>
        {stage === "centered" && (
          <motion.div
            key="simple-intro-splash"
            initial={{ opacity: 1 }}
            exit={{ opacity: 0, scale: 0.98 }}
            transition={{ duration: 0.45, ease: "easeInOut" }}
            className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black text-white px-4 pointer-events-none"
          >
            <motion.div
            ...
            >
              <AvatarCard isInitialFlipped={isInitialFlipped} />
              ...
              <div className="flex flex-col items-center text-center gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <Sparkles className="w-5 h-5 text-amber-400 animate-pulse inline-block" />
                </div>
                <h2 className="text-lg md:text-xl font-medium text-zinc-400">
                  Web2 & Web3 Developer
                </h2>
              </div>
            </motion.div>
      ...
      <div className="w-full flex flex-col items-center">
        <main className="min-h-screen flex flex-col justify-between items-center pt-24 md:pt-32 pb-12 px-4 sm:px-6 md:px-8 relative w-full max-w-2xl mx-auto space-y-10 md:space-y-12">
          {/* Top Hero Header Block - Vertically centered with image horizontal midpoint */}
          <div className="flex flex-col-reverse sm:flex-row items-center sm:items-center justify-between gap-6 sm:gap-8 md:gap-10 w-full">
          ...
          {/* Hero Content */}
          <div className="flex flex-col items-center sm:items-start w-full space-y-10 md:space-y-12">
